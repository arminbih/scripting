...
$mainFolderPath = "\\fileserver\UPLATNI RACUNI"
...
foreach ($file in $files) {
    ...
    if (Test-Path $folderPath) {
        # Move the file to the corresponding folder, **using -Force to overwrite**
        $destination = Join-Path -Path $folderPath -ChildPath $file.Name
        Move-Item -Path $file.FullName -Destination $destination -Force

        # Log the move operation
        $logEntry = "Moved (Overwrote) $($file.Name) to $($folderPath)"
